...
public class CreditController implements V1Api {
        ...
        /**
         * @return Mono Response Entity of Mono of Payment response to transactions
         * @param paymentRequest request to process payment
         */
        @Override
        public Mono<ResponseEntity<PaymentResponse>> payment(final @Valid Mono<PaymentRequest> paymentRequest,
                        final ServerWebExchange exchange) {
                return paymentRequest
                                .flatMap(request -> creditService.payment(request)
                                .map(paymentResponse -> {
                                        return ResponseEntity
                                                        .status(HttpStatus.OK)
